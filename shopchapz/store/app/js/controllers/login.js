...
(function () {
    ...
    login.$inject = ['$rootScope', '$scope', '$timeout', '$state', '$localStorage', 'loginSignupService', '$location', '$mdDialog'];
    function login($rootScope, $scope, $timeout, $state, $localStorage, loginSignupService, $location, $mdDialog) {
        ...
        $scope.login = function () {
            ...
            loginSignupService.login($scope.user).success(function (response) {
                ...
                if (response.status == 105 || response.status == "105") {
                    ...
                    localStorage.setItem("storeadmintoken", response.data.user.access_token);
